...
function createModal(bbsSeq, key){
	$.ajax({
	      type: "POST",
	      url: "${root}/aboard",
	      data: {act: key, bbsSeq: bbsSeq},
	      dataType: "json",
...
function boarddelete(bbsSeq){
	document.location.href = "${root}/pboard?act=boarddelete&bbsSeq="+ bbsSeq;
}

function boardsearch() {
	var word = document.getElementById("word").value;
	document.location.href = "${root}/pboard?act=boardlist&word=" + word;
}
</script>
</head>
<body>
					...
					<div class="dataTables_wrapper form-inline no-footer">
										...
										<c:forEach var="boardlist" items="${boardList}">
											<tr>
												...
												<td>
													...
													onclick="javascript:createModal('${boardlist.bbsSeq}','update');">&nbsp;&nbsp;
													<input type="button" value="삭제" id="btndelete"
													class="btn btn-danger btn-xs"
													data-toggle="modal" data-target="#DeleteModal"
													onclick="javascript:boarddelete('${boardlist.bbsSeq}');">
												</td>
											</tr>
										</c:forEach>
									</c:if>
						...
						<div class="dt-toolbar-footer">
										...
										<%-- <c:choose>
											<c:when test="${totalCount%listNum == 0}">
												<c:forEach var="page" begin="1" end="${totalCount/listNum}">
													<li class="active"><a href="${root}/pboard?act=boardlist&pg=${page}">${page}</a></li>&nbsp;&nbsp;
											</c:forEach>
											</c:when>
											<c:otherwise>
												<c:forEach var="page" begin="1"
													end="${totalCount/listNum + 1}">
													<li data="${page}"><a href="${root}/pboard?act=boardlist&pg=${page}">${page}</a></li>&nbsp;&nbsp;
												</c:forEach>
											</c:otherwise>
										</c:choose> --%>
										${navigator}
							...
							<div id="buttonSet" class="col-sm-4 text-right">
								...
								<!-- <input type="button" value="등록" class="btn btn-info btn-sm" 
								data-toggle="modal" data-target="#myFullsizeModal"
								onclick="javascript:createModal('','write');"> -->
							</div>
						</div>
					</div>
				</div>
...
<!-- ============== 모달 팝업 ================================= -->
<!-- <div class="modal modal-center fade" id=myFullsizeModal tabindex="-1" role="dialog" aria-labelledby="myFullsizeModalLabel" aria-hidden="true" >
